...
impl WikiPlugin {
    ...
    async fn regenerate_autogenerated_sections(&self, nvim: &mut Neovim<Compat<tokio::fs::File>>) -> Result<(), Error> {
        nvim_eval_and_cast!(current_note_full_path, nvim, r#"expand("%:p")"#, as_str, "vim function expand( should always return a string");
        let current_note_full_path = Path::new(current_note_full_path);
        let current_buf_parent_dir = current_note_full_path
            .parent()
            .ok_or_else(|| format!("could not get parent of current buffer because current buffer path is {current_note_full_path:?}"))?;
        let current_buf = nvim.get_current_buf().await?;

        let autogen_start_marker_regex = Regex::new(r#"\<zet_autogenerate\>\s*(\w+)(.*)"#).expect("autogenerate start marker regex should be valid");
        let autogen_end_marker_regex = Regex::new(r#"\<zet_autogenerate_end\>"#).expect("autogenerate end marker regex should be valid");
        ...
        loop {
            ...
            let end_line = if let Some(end_line) = end_line {
                ...
                current_buf
                    .set_lines((start_line_nr + 1).try_into()?, (start_line_nr + 1).try_into()?, false, vec!["zet_autogenerate_end".to_string()])
                    .await?;
                start_line_nr + 1
            };
