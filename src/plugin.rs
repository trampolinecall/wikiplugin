...
use crate::{
    ...
    plugin::{
        messages::Message,
        note::{Note, PhysicalNote, ScratchNote, Tag},
    },
};
...
impl nvim_rs::Handler for WikiPlugin {
    ...
    async fn handle_notify(&self, name: String, args: Vec<nvim_rs::Value>, mut nvim: Neovim<Compat<tokio::fs::File>>) {
        ...
        let result = match message {
            ...
            // TODO: why are there so many insert link messages (especially because the lua side only ever uses one of them (also these need cleaning up (and ideally would not use Note::parse_from_filepath)))
            ...
            Message::OpenMaintenanceIndex {} => self.open_maintenance_index(&mut nvim).await,
...
impl WikiPlugin {
    ...
    async fn regenerate_autogenerated_sections(&self, nvim: &mut Neovim<Compat<tokio::fs::File>>) -> Result<(), Error> {
        ...
        // repeatedly use matchlist to find all of the autogenerated sections
        ...
        loop {
            ...
            // end_line is None if there is no ending marker in the file
            let mut end_line = buf_lines.iter().enumerate().skip(start_line_nr + 1).find(|(_, line)| autogen_end_marker_regex.is_match(line));

            let next_start_line = buf_lines.iter().enumerate().skip(start_line_nr + 1).find(|(_, line)| autogen_start_marker_regex.is_match(line));

            if let Some(next_start_line) = next_start_line {
                if let Some(el) = end_line {
                    // if end_line > next_start_line, end_line is pointing at the end marker of the next
                    // autogeneration region, meaning that the current one doesn't have an end marker, so
                    // we need to set end_line to None to reflect that
                    if el > next_start_line {
                        end_line = None;
                    }
                }
            }

            let end_line = if let Some(end_line) = end_line {
                end_line.0
            } else {
                current_buf
                    .set_lines(
    ...
    async fn open_maintenance_index(&self, nvim: &mut Neovim<Compat<tokio::fs::File>>) -> Result<(), Error> {
        async fn invalid_title(config: &Config, nvim: &mut Neovim<Compat<tokio::fs::File>>, note: &PhysicalNote) -> bool {
            note.parse_markdown(config, nvim)
                .await
...

        {
            ...
            for note in &notes {
                if invalid_title(&self.config, nvim, note).await {
                    invalid_titles.push(format!(
                        "- [unnamed {}]({})",
                        index,
                        links::format_link(&self.config, &current_note, &note.path(&self.config))?
                    ));
                    index += 1;
                }
            }
