...
use crate::plugin::{filetype::{Filetype, FiletypeRegistry}, note::{Note, PhysicalNote, Tag}};
...
#[derive(Debug)]
pub struct FiletypeDictMissingKey(&'static str);
impl std::error::Error for FiletypeDictMissingKey {}
impl std::fmt::Display for FiletypeDictMissingKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "filetype dict is missing key {}", self.0) // TODO: say which filetype dict
    }
}
#[derive(Debug)]
...
error_union! {
    pub enum ConfigParseError {
        ...
        FiletypeDictMissingKey(FiletypeDictMissingKey),
...
impl Config {
    pub fn parse_from_dict(dict: Dictionary) -> Result<Config, ConfigParseError> {
        // TODO: figure out a better way to do this than to have 2 basically identical functions
        fn get_from_config_dict<T: nvim_oxi::conversion::FromObject>(dict: &Dictionary, key: &'static str) -> Result<T, ConfigParseError> {
            Ok(T::from_object(dict.get(key).ok_or(ConfigDictMissingKey(key))?.clone())?)
        }
        fn get_from_filetype_dict<T: nvim_oxi::conversion::FromObject>(dict: &Dictionary, key: &'static str) -> Result<T, ConfigParseError> {
            Ok(T::from_object(dict.get(key).ok_or(FiletypeDictMissingKey(key))?.clone())?)
        }

        let home_path: PathBuf = get_from_config_dict::<String>(&dict, "home_path")?.into();
        if !home_path.is_absolute() {
            Err(HomePathNotAbsolute)?;
        }

        let mut filetypes = FiletypeRegistry::new();
        for ft_dict in get_from_config_dict::<Vec<Dictionary>>(&dict, "filetypes")? {
            let ft = Filetype {
                extension: get_from_filetype_dict(&ft_dict, "extension")?,
            };
            filetypes.register(ft);
        }

        let c = Config {
            home_path,
            note_id_timestamp_format: get_from_config_dict(&dict, "note_id_timestamp_format")?,
            date_format: get_from_config_dict(&dict, "date_format")?,
            time_format: get_from_config_dict(&dict, "time_format")?,
            filetypes,
        };
        Ok(c)
    }
...
pub fn new_note(config: &Config, template: Option<String>, directories: Vec<String>, focus: bool) -> Result<Note, NewNoteError> {
    ...
    let note_path = {
        ...
        p.push(&note_id);
        p.set_extension("md"); // TODO: allow custom extensions
        p
    };
    ...
    Ok(Note::new_physical(directories, note_id, todo!())) // TODO: implement this correctly
}
...
pub fn regenerate_autogenerated_sections(config: &Config) -> Result<(), AutogenerateError> {
    ...
    // TODO: there seems to be a bug that it will only generate the first one
        ...
        negative_one_to_option(api::eval(&format!("match(getline(0, '$'), '{autogen_start_marker_regex}', 0, {match_index})"))?)
    {
        ...
        let replacement = match autogenerate_command {
            "index" => {
                ...
                type ComparatorTuple<'fr> = (PhysicalNote<'fr>, ::markdown::mdast::Node, Option<yaml_rust::Yaml>, Option<String>);
            ...
            "listtag" => {
                let filter_tag = Tag::parse_from_str(autogenerate_arguments.first().copied().unwrap_or(""));
                // TODO: implement sort_by and don't duplicate it with index above
                // let sort_by = autogenerate_arguments.get(1).copied().unwrap_or("title");

                let mut notes_under_tag = Vec::new();
                for note in list_all_physical_notes(config)? {
                    let md = markdown::parse_markdown(&note.read_contents(config)?)?; // TODO: don't error on this?
                    let frontmatter = markdown::parse_frontmatter(&md).ok();
                    let tags = frontmatter.as_ref().map(markdown::get_tags).unwrap_or(Ok(Vec::new()))?;
                    if tags.iter().any(|note_tag| note_tag.included_under(&filter_tag)) {
                        let title = frontmatter.as_ref().and_then(|f| markdown::get_title(f).ok());
                        notes_under_tag.push((note, title))
                    }
                }

                // TODO: don't duplicate this code with sort_by above
                type ComparatorTuple<'fr> = (PhysicalNote<'fr>, Option<String>);
                let comparator = (&|(a, a_title): &ComparatorTuple, (b, b_title): &ComparatorTuple| {
                    if a_title.is_none() || b_title.is_none() {
                        a.id.cmp(&b.id)
                    } else {
                        a_title.cmp(b_title)
                ...
                notes_under_tag.sort_by(comparator);

                // TODO: make a helper function to format a list of links
                let mut result = Vec::new();
                for (file, title) in notes_under_tag {
                    let link_path = links::format_link_path(config, &current_note, &file.path(config))?;
                    result.push(format!("- [{}]({})", title.unwrap_or("".to_string()), link_path));
                }
            ...
            "backlinks" => {
                ...
                for other_note in list_all_physical_notes(config)? {
                    if current_note.get_id() == Some(&other_note.id) {
                        continue;
                    }
