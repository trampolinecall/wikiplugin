        ...
        negative_one_to_option(api::eval(&format!("match(getline(0, '$'), '{}', 0, {})", autogen_start_marker_regex, match_index))?)
    {
        ...
        let end_line = {
            let end_marker_line_index =
                negative_one_to_option(api::eval(&format!("match(getline(0, '$'), '{}', {})", autogen_end_marker_regex, start_line_index))?);

            let next_start_line_index =
                negative_one_to_option(api::eval(&format!("match(getline(0, '$'), '{}', {})", autogen_start_marker_regex, start_line_index))?);
