...
pub fn regenerate_autogenerated_sections(config: &Config) -> Result<(), Error> {
        ...
        negative_one_to_option(api::eval(&format!("match(getline(0, '$'), {}, 0, {})", autogen_start_marker_regex, match_index))?)
    {
        ...
        let end_line = {
            let end_marker_line =
                negative_one_to_option(api::eval(&format!("match(getline(0, '$'), {}, {})", autogen_end_marker_regex, start_line))?);

            let next_start_line =
                negative_one_to_option(api::eval(&format!("match(getline(0, '$'), {}, {})", autogen_start_marker_regex, start_line))?);

            let mut insert_end_line = || {
                current_buf.set_lines(start_line + 1..start_line + 1, false, vec!["wikiplugin_autogenerate_end".to_string()])?;
                Ok::<_, Error>(start_line + 1)
            };
        ...
        if let Some(replacement) = replacement {
            current_buf.set_lines((start_line + 1)..end_line, false, replacement)?;
        }
